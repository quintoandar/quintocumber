...
browserstack_default_config = YAML.safe_load(File.read(File.join(File.dirname(__FILE__), './browserstack.yml')))

browserstack_project_config_filename = File.join(Dir.pwd, './browserstack.yml')
if File.file?(browserstack_project_config_filename)
  browserstack_project_config = YAML.safe_load(File.read(browserstack_project_config_filename)) || {}
else
